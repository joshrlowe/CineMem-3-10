...
def plane_plane_intersection(
    ...
    Returns:
        tuple of (line_point, line_vec), or None if the planes are (near-)parallel.
    """
    # (3,)
    plane_norm_vec1 = rot1[:, -1]
    norm1 = np.linalg.norm(plane_norm_vec1)
    if norm1 < 1e-12:
        logger.error("plane_norm_vec1 has near-zero norm; cannot compute plane intersection.")
        return None
    plane_norm_vec1 = plane_norm_vec1 / norm1

    plane_norm_vec2 = rot2[:, -1]
    norm2 = np.linalg.norm(plane_norm_vec2)
    if norm2 < 1e-12:
        logger.error("plane_norm_vec2 has near-zero norm; cannot compute plane intersection.")
        return None
    plane_norm_vec2 = plane_norm_vec2 / norm2

    line_vec = np.cross(plane_norm_vec1, plane_norm_vec2)
    line_vec_norm = np.linalg.norm(line_vec)
    if line_vec_norm < 1e-8:
        logger.error("2C and 4C planes are (near-)parallel; cannot compute intersection line.")
        return None
...
def get_sax_center(
    ...
    """
    # 2C/4C intersection
    result = get_lax_2c_4c_plane_intersection(lax_2c_image, lax_4c_image)
    if result is None:
        logger.error("Cannot compute 2C/4C plane intersection; returning None.")
        return None
    line_point, line_vec = result
