...
def load_dicom_folder(slice_dirs: list[Path], path: Path) -> sitk.Image:  # noqa: C901
    ...
    # Compute axis_z from two successive slice positions when possible.
    # Fall back to the cross product of axis_x and axis_y when:
    #   - there is only one slice, OR
    #   - the first two slices share the same position (norm ≈ 0).
    axis_z = None
    slice_dist = 0.0
    if z >= 2:
        ...
        pos_ul2[:2] = -pos_ul2[:2]
        diff = pos_ul2 - pos_ul
        slice_dist = float(np.linalg.norm(diff))
        if slice_dist > 1e-6:
            axis_z = diff / slice_dist
    ...
    if axis_z is None:
        ...
        if norm > 1e-6:
            ...
            # Degenerate case: axis_x and axis_y are parallel; use a default z direction.
            logger.warning("axis_x and axis_y are (near-)parallel; using default z-axis [0,0,1].")
            axis_z = np.array([0.0, 0.0, 1.0])

    # Determine the z spacing
    if hasattr(d, "SpacingBetweenSlices"):
        dz = float(d.SpacingBetweenSlices)
    elif slice_dist > 1e-6:
        dz = slice_dist
    elif hasattr(d, "SliceThickness"):
        dz = float(d.SliceThickness)
    else:
        dz = 1.0
        logger.warning(f"Cannot determine z-spacing for {path}; defaulting to 1.0 mm.")
    ...
    # Orthonormalize the rotation part of the affine so that SimpleITK
    # (which requires orthonormal direction cosines) can read the NIfTI back.
    rotation = affine[:3, :3]  # columns = axis_x*dx, axis_y*dy, axis_z*dz
    spacings = np.linalg.norm(rotation, axis=0)  # recover per-axis spacing
    rotation_normed = rotation / spacings[np.newaxis, :]  # unit direction vectors
    # QR decomposition gives the nearest orthonormal basis
    q, _ = np.linalg.qr(rotation_normed)
    affine[:3, :3] = q * spacings[np.newaxis, :]  # re-apply spacing
